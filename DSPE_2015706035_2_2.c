// 2주차_2015706035_홍세준 Assignment#2
...
int main()
{
	...
	BYTE **img_in = 0, **img_out = 0;			// Pointers for inpu and output
	float temp = 0;
	int i = 0, j = 0, m = 0, n = 0;
	...
	img_in = (BYTE**)malloc(sizeof(BYTE*)*(HEIGH + 6));		// input memory allocation
	for (i = 0; i < HEIGH + 6; i++)
	{
	...
	fp_out = fopen("[Output_ave7]Lena(512x512).raw", "wb");	// Output file open(.raw)
	if (fp_out == NULL)
	{
		printf("File open failed\n");
	}
	...
	for (i = 0; i < HEIGH + 6; i++)		// Memory free
	{
		free(img_in[i]);
		if (i < HEIGH)
			free(img_out[i]);
